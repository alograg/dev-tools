...
class TestMakeCommand extends Maker
{
    use MakerTrait;
    /**
...
make:test {name : The name of the class} 
    ...
    {--sp|stub-path : Path relative a stubs repo}
TEXT;
    ...
    protected function buildClass($name)
    {
        ...
        if ($this->option('model')) {
            ...
            if ($this->option('parent')) {
                $replace = array_merge($replace, [
                    'dummy_action_index'   => 'route(\'' . $this->getRouteName($name) . '.index\', [ \'' . $replace['parent_dummy_model_variable'] . '\' => $' . $replace['parent_dummy_model_variable'] . '->getKey() ])',
                    'dummy_action_store'   => 'route(\'' . $this->getRouteName($name) . '.store\', [ \'' . $replace['parent_dummy_model_variable'] . '\' => $' . $replace['parent_dummy_model_variable'] . '->getKey() ])',
                    'dummy_action_show'    => 'route(\'' . $this->getRouteName($name) . '.show\', [ \'' . $replace['parent_dummy_model_variable'] . '\' => $' . $replace['parent_dummy_model_variable'] . '->getKey(), \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey() ])',
                    'dummy_action_update'  => 'route(\'' . $this->getRouteName($name) . '.update\', [ \'' . $replace['parent_dummy_model_variable'] . '\' => $' . $replace['parent_dummy_model_variable'] . '->getKey(), \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey()  ])',
                    'dummy_action_destroy' => 'route(\'' . $this->getRouteName($name) . '.destroy\', [ \'' . $replace['parent_dummy_model_variable'] . '\' => $' . $replace['parent_dummy_model_variable'] . '->getKey(), \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey()  ])',
                ]);
            } else {
                $replace = array_merge($replace, [
                    'dummy_action_index'   => 'route(\'' . $this->getRouteName($name) . '.index\')',
                    'dummy_action_store'   => 'route(\'' . $this->getRouteName($name) . '.store\')',
                    'dummy_action_show'    => 'route(\'' . $this->getRouteName($name) . '.show\', [ \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey() ])',
                    'dummy_action_update'  => 'route(\'' . $this->getRouteName($name) . '.update\', [ \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey() ])',
                    'dummy_action_destroy' => 'route(\'' . $this->getRouteName($name) . '.destroy\', [ \'' . $replace['dummy_model_variable'] . '\' => $' . $replace['dummy_model_variable'] . '->getKey() ])',
                ]);
            }
        }
        ...
        return str_replace(array_keys($replace), array_values($replace),
            $this->replaceUserNamespace(parent::buildClass($name)));
    }
        ...
        if (!$this->files->exists($this->getAppPath($parentModelClass))) {
            if ($this->confirm("A {$parentModelClass} model does not exist. Do you want to generate it?", true)) {
                $this->call('make:model', [
                    'name' => str_replace($this->laravel->getNamespace(), '', $parentModelClass),
                    '-m'   => true,
                    '-f'   => true,
                ]);
            }
    ...
    protected function buildModelReplacements(array $replace)
    {
        ...
        if (!$this->files->exists($this->getAppPath($modelClass))) {
            if ($this->confirm("A {$modelClass} model does not exist. Do you want to generate it?", true)) {
                $this->call('make:model', [
                    'name' => str_replace($this->laravel->getNamespace(), '', $modelClass),
                    '-m'   => true,
                    '-f'   => true,
                ]);
            }
